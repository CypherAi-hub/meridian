...
/** Plugin unit tests must not leak the live app's site.json / public/og.jpg. */
function isolated(ctx = {}) {
  const cwd = ctx.cwd ?? mkdtempSync(join(tmpdir(), "grok-pwa-iso-"));
  return { site: {}, cwd, ...ctx, cwd, site: ctx.site !== undefined ? ctx.site : {} };
}
...
test("document title entities are not double-escaped on og:title", () => {
  const amp = ["&", "amp", ";"].join("");
  const html = `<html><head><title>Cats ${amp} Dogs</title></head></html>`;
  const out = injectGrokPwaHead(html, isolated());
  assert.match(out, new RegExp(`property="og:title" content="Cats ${amp} Dogs"`));
  assert.doesNotMatch(out, new RegExp(`Cats ${amp}amp; Dogs`));
});
